    ...
    def create_data_frame(
        ...
        for filename_no_extension, sentence in tqdm(
            ...
            for word_index, word in enumerate(tg.getTier("words")):
                ...
                if self.path_to_corpus == "../../mini_corpus" or self.path_to_corpus == "../../miniatur_korpus":
                    if  not os.path.exists(f"manual_tests/synthesized_clips_{self.language}"):
                        os.mkdir(f"manual_tests/synthesized_clips_{self.language}")
                    sf.write(f"manual_tests/synthesized_clips_{self.language}/{word.label}.wav", wav_syn, wav_syn_sr)
